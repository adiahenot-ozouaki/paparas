...
/** Presets solo — Partie rapide (compétitif court). */
const QUICK_PRESET = {
  baseStake: 500,
...
/** Presets solo — Entraînement (mise basse, paquet court). */
...
export default function GameModeScreen({ onNavigate }: { onNavigate: (s: Screen) => void }) {
  ...
  function openOnlineJoin(code?: string) {
    requireAuthThen(() => {
      setOnlineLobbyIntent('join')
      setPendingJoinCode(code ?? null)
      onNavigate('onlineLobby')
    })
  }
  ...
  return (
    ...
    >
        ...
        >
          <div
            style={{
              ...
              width: 4,
              height: '100%',
              background: 'linear-gradient(180deg, #176B50, transparent)',
            }}
          />
          <div style={{ paddingLeft: 10 }}>
            ...
            <p style={{ color: '#A9B0B7', fontSize: 13, margin: '0 0 14px', lineHeight: 1.45 }}>
              Une seule entrée : créez une table privée (code à partager) ou rejoignez avec un code / une table
              ouverte.
            </p>
            <div style={{ display: 'flex', gap: 10, flexWrap: 'wrap' }}>
              <button
                ...
                onClick={openOnlineCreate}
                style={{ flex: 1, minWidth: 120, padding: '12px 14px', borderRadius: 12, fontSize: 13 }}
              >
                Créer une table
              </button>
              <button
      ...
      <div style={{ padding: '8px 20px 24px' }}>
        ...
        {listError && <p style={{ color: '#C94B4B', fontSize: 12, margin: '0 0 8px' }}>{listError}</p>}
        {openTables.length === 0 ? (
          <div
            style={{
              background: 'rgba(255,255,255,0.04)',
