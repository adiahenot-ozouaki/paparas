...
const VARIANTS = [
  { variant: '9' as const, expectedSize: 27 },
  { variant: '10' as const, expectedSize: 31 },
  { variant: 'as' as const, expectedSize: 35 },
]

test('buildDeck crée les bonnes tailles de paquet', () => {
  for (const { variant, expectedSize } of VARIANTS) {
    assert.equal(buildDeck(variant).length, expectedSize)
  }
})
...
test('buildDeck retire le plus gros pique et conserve des cartes uniques', () => {
  for (const { variant, expectedSize } of VARIANTS) {
    ...
    assert.ok(!deck.some((card) => card.suit === '♠' && card.value === deck.find((c) => c.suit === '♥')?.value))
  }
})
...
test('dealHands distribue 5 cartes à chaque joueur sans perte de cartes', () => {
  ...
  const dealtKeys = hands.flat().map((card) => `${card.suit}-${card.value}`)
  assert.equal(new Set(dealtKeys).size, 20)
  assert.deepEqual(
    hands.flat(),
    deck.slice(0, 20),
  )
...
test('shuffleDeck produit des ordres différents sur plusieurs mélanges', () => {
  ...
  const signatures = new Set(
    Array.from({ length: 5 }, () => shuffleDeck(deck).map((card) => `${card.suit}-${card.value}`).join('|')),
  )
